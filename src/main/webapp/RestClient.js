 ...
 * Click nbfs://nbhost/SystemFileSystem/Templates/ClientSide/rest.js to edit this template
 */
...
// Model for Productos entity
    models.Productos = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/CrudProductos2021-22SUBIR/webresources/generic/producto/",
        idAttribute: 'id',
        defaults: {
            precio: "",
            categoria: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/CrudProductos2021-22SUBIR/webresources/generic/producto/';
            }
            if (method === 'update') {
                options.url = 'http://localhost:8080/CrudProductos2021-22SUBIR/webresources/generic/producto/';
            }
    ...
    // Collection class for Productos entities
    models.ProductosCollection = Backbone.Collection.extend({
        model: models.Productos,
        url: "http://localhost:8080/CrudProductos2021-22SUBIR/webresources/generic/productos/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProductosCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-productos-list-itemis template identifier for item
                        templateName: '#tpl-productos-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.productos = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    ...
                    // tpl-productos-details is template identifier for chosen model element
                    templateName: '#tpl-productos-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Productos();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-productos-details is a template identifier for chosen model element
                templateName: '#tpl-productos-details',
